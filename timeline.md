# Investigation Timeline — Elastic Security Lab 01

> All timestamps below are based on the Elastic Discover results captured during the lab session on September 20, 2026. The timeline records observed telemetry and investigation actions. It does not assume causality between events unless supported by the available evidence.
...
- Elastic Defend was collecting endpoint telemetry.
- PowerShell process activity was visible in Discover.
- ES|QL could identify PowerShell activity.
- `-EncodedCommand` could be used as a hunting condition.
- The encoded event had PID `9264`.
- The recorded parent process was `pwsh.exe`.
- Parent-process information was not consistently populated across all PowerShell events.
- The encoded command was generated as part of a controlled benign simulation.
...
- A confirmed `WINWORD.EXE -> powershell.exe` relationship.

The timeline therefore supports a finding of **controlled encoded PowerShell activity observed through Elastic endpoint telemetry**, rather than a confirmed malicious incident.
