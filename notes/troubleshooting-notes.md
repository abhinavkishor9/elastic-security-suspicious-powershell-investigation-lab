# Troubleshooting Notes — Elastic Security Lab 01
...
## 10. General Troubleshooting Method Used
...
This approach prevents troubleshooting from becoming guesswork.

## Key Troubleshooting Lessons

- Identify the query language before writing queries.
- `Search field names` is not the ES|QL query editor.
- Validate the data source before troubleshooting a detection.
- Confirm that the field used by a detection actually exists.
- Confirm that the field is populated.
- Use a short time range during controlled testing.
- Do not interpret missing fields as proof that an event did not occur.
- Do not create findings that are not supported by telemetry.
- Treat suspicious indicators as investigation leads.
