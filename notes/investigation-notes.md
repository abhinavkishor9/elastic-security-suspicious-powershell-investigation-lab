# Investigation Notes — Elastic Security Lab 01

## Investigation Overview

This investigation focused on PowerShell process activity collected by Elastic Defend from a Windows endpoint.

The objective was to validate endpoint telemetry, establish a normal PowerShell baseline, identify encoded PowerShell execution, and investigate the available process and parent-process context.

The investigation was performed using Elastic Discover with ES|QL.
...
Elastic successfully captured PowerShell endpoint telemetry and allowed the activity to be searched using ES|QL.

The use of `-EncodedCommand` provided a useful hunting signal and demonstrated how process command-line telemetry can be used to identify potentially interesting PowerShell activity.

The parent process for the identified encoded event was `pwsh.exe`. Other PowerShell events did not consistently contain parent-process information.

The evidence supports a controlled encoded PowerShell execution but does not support a conclusion of malicious compromise.

The main lesson is that suspicious indicators should lead to further investigation rather than being treated as conclusions.
